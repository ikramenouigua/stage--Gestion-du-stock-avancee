...
<body>
  ...
  <div class="container">
    ...
    @foreach($avoire as $a)
      <p> Date :  {{$a->date_avoire}} </p> 
    
    @endforeach
    @foreach($commandes as $cmd)
       <p>  N°Fournisseur : {{$cmd->id_fournisseur}} </p>
       @endforeach
       ...
       <hr/>
          @foreach($fournisseurs as $f)
             <p>Nom du fournisseur : {{$f->nom_fournisseur}}
                <p>Telephone: {{$f->tel}}</p>
               <p> 44003</p> 
               @endforeach
         </div>
...
<p>Total TTC en dirhams :    {{$c->prix_total+$c->prix_total*0.2}} DH</p> 
@endforeach
<br/><br/>
    </div>
